...
/**
 * Safely render an HTML element to a canvas using html2canvas,
 * completely bypassing the "Attempting to parse an unsupported color function 'oklab'" and 'oklch' errors
 * generated by Tailwind CSS v4 in nested stylesheet variables.
 */
async function safelyRenderCanvas(element: HTMLElement, options: any): Promise<HTMLCanvasElement> {
  // Capture original descriptors for style rules getters
  ...
  const patchSheet = (originalDescriptor: PropertyDescriptor | undefined) => {
    if (!originalDescriptor || !originalDescriptor.get) return undefined;
    return {
      get() {
        try {
          const rawRules = originalDescriptor.get!.call(this);
          if (!rawRules) return rawRules;
          ...
          for (let i = 0; i < rawRules.length; i++) {
            ...
            try {
              if (rule.cssText && (rule.cssText.includes('oklab') || rule.cssText.includes('oklch'))) {
                // Skip rules parsing that contains the unsupported css oklab/oklch parameters entirely!
                continue;
              }
              filtered.push(rule);
            } catch (e) {
              // Gracefully pass-through rule if any CORS cross-origin permission error occurs
              filtered.push(rule);
            }
          }

          // Build our own array-like mock CSSRuleList that satisfies both length check and index lookup
          const mockList = Object.create(CSSRuleList.prototype);
          Object.defineProperty(mockList, 'length', { value: filtered.length });
          filtered.forEach((rule, index) => {
            Object.defineProperty(mockList, index, { value: rule, enumerable: true });
          });
          mockList.item = (index: number) => filtered[index] || null;
          return mockList;
        } catch (err) {
          // Fallback if rule fetching throws
  ...
  // Perform the temporary global hook patch on CSSStyleSheet getters, getComputedStyle and getPropertyValue
  try {
    if (originalCssRules) {
      Object.defineProperty(CSSStyleSheet.prototype, 'cssRules', patchSheet(originalCssRules)!);
    }
    if (originalRules) {
      Object.defineProperty(CSSStyleSheet.prototype, 'rules', patchSheet(originalRules)!);
    }
  } catch (e) {
    console.warn('Failed to apply html2canvas stylesheet safety patches:', e);
  }
  ...
  try {
    ...
    console.warn('Failed to apply getter safety patches:', e);
  }

  try {
    let h2c: any = html2canvas;
    if (typeof h2c !== 'function') {
      if (h2c && typeof h2c.default === 'function') {
        h2c = h2c.default;
      } else {
        try {
          const html2canvasModule = await import('html2canvas');
          h2c = html2canvasModule.default || html2canvasModule;
        } catch (dynErr) {
          console.error('Failed to dynamically import html2canvas:', dynErr);
        }
      }
    }

    if (typeof h2c !== 'function') {
      throw new Error('html2canvas is not a callable function');
    }

    const canvas = await h2c(element, options);
    return canvas;
  } finally {
    // Revert the temporary global hooks safely under a complete recovery block
    try {
      if (originalCssRules) {
      ...
      console.warn('Failed to revert html2canvas stylesheet safety patches:', e);
    }
  }
}

/**
 * All-environment compatible robust clipboard text copy helper
 */
...
export default function App() {
  ...
  // iOS-specific save preview mask overlay states
  const [showSavePreview, setShowSavePreview] = useState(false);
  const [previewSaveImage, setPreviewSaveImage] = useState<string | null>(null);

  // Device environment recognition helper
  const getDevice = () => {
    ...
    if (appState === 'reveal' && chosenAnswer) {
      ...
      const timer = setTimeout(async () => {
        ...
        if (element) {
          ...
          try {
            const canvas = await safelyRenderCanvas(element, {
              useCORS: true,
              scale: 2, // Standard High-DPI 2x retina crispness is highly stable and avoids mobile device memory caps!
  ...
  // ========== 高清保存神签至相册（支持微信/iOS/Android/PC自适应判断） ==========
  const handleSaveToLocal = async (imageSource: string | null, customFilename = '答案之书_命运神签.png') => {
    let imgToSave = imageSource;
    
    // 如果没有传入特定的图片源，尝试获取当前卡片的生成的 3x 高清图片或者画布图片
    if (!imgToSave) {
      if (generatedImg) {
        imgToSave = generatedImg;
      } else if (generatedImage) {
        imgToSave = generatedImage;
      } else {
        showToast('✨ 正在为您加持凝聚高清画屏中，请稍等一瞬... ✨');
        // 如果当时图片还没合拢完成，我们在此尝试现场转换
        const element = document.getElementById('destiny-book-card');
        if (element) {
          setIsGeneratingImg(true);
          try {
            ...
            console.error('Save live canvas error:', err);
            setIsGeneratingImg(false);
            showToast('❌ 凝聚画卷暂时受阻，请稍后重试。');
            return;
          }
        } else {
          showToast('❌ 暂无可供保存的命运画卷，请重试。');
          return;
        }
      }
    }

    const { isWechat, isIos, isAndroid } = getDevice();

    // 1. 微信内置浏览器打开情况
    if (isWechat) {
      showToast('💡 微信内不支持由于安全机制的直接下载，请「长按」上方卡片图保存，或点击右上角「···」选择「在浏览器打开」一键保存图片哦 ✨');
      // 如果是微信里的苹果设备，可以贴心地调起大图遮罩，让其可长按
      if (isIos) {
        setPreviewSaveImage(imgToSave);
        setShowSavePreview(true);
      }
      return;
    }

    // 2. 安卓原生浏览器等环境 -> 触发一键标准打包直接下载
    if (isAndroid) {
      try {
        ...
        showToast('✨ 已成功触发下载！请在您的手机「通知栏下载/相册/文件管理」中查看保存的神签图片哦 ✨');
        grantShareBonus(); // 给用户赠送福缘 bonus
      } catch (e) {
        console.error('Android download process hit sandbox, fall back to iOS style view:', e);
        setPreviewSaveImage(imgToSave);
        setShowSavePreview(true);
      }
      return;
    ...
    // 3. 苹果 iOS 环境 (程序性 click 下载会被 Safari 拦截阻挡) -> 弹出沉浸式大图遮罩预览，提示用户长按保存
    if (isIos) {
      setPreviewSaveImage(imgToSave);
      setShowSavePreview(true);
      return;
    ...
    // 4. PC/Mac 或其他标准浏览器设备 -> 触发原生的 href 模拟下载
    try {
      const link = document.createElement('a');
      link.href = imgToSave!;
      link.download = customFilename;
      ...
      link.click();
      document.body.removeChild(link);
      showToast('✨ 命运神签已成功保存至您的本地设备，快去相册中查看吧 ✨');
    } catch (e) {
      console.error('Fallback to image element', e);
      setPreviewSaveImage(imgToSave);
      setShowSavePreview(true);
    }
  };
  ...
  useEffect(() => {
    ...
    const resetDone = localStorage.getItem('answers_book_reset_to_3_v7');
    if (!resetDone) {
      localStorage.setItem('answers_book_daily_used', '0');
      localStorage.setItem('answers_book_daily_bonus', '0');
      localStorage.setItem('answers_book_reset_to_3_v7', 'true');
    }
  ...
  const handleAttuneStart = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    // Explicitly wake/unlock the AudioContext synchronously within this user gesture event callback (required for iOS Safari)
    synth.unlock();
  ...
  return (
    <div className="relative min-h-screen text-slate-100 font-sans overflow-x-hidden select-none flex flex-col justify-between">
          ...
          {appState === 'reveal' && chosenAnswer && (
            ...
            >
              ...
              >
                ...
                {generatedImg && (
                  <img
                    ...
                    src={generatedImg}
                    alt="长按保存命运神签"
                    className="absolute inset-0 w-full h-full opacity-[0.01] pointer-events-auto z-30 cursor-default select-none object-stretch"
                    style={{ 
              ...
              <motion.div
                ...
                onClick={() => handleSaveToLocal(generatedImg)}
              ...
              >
                ...
                <span className="font-serif text-[10.5px] text-amber-200/90 tracking-[0.14em] uppercase select-none font-medium">
                  {isGeneratingImg ? "正在极尽凝聚天命画卷..." : "✨ 命理已结！点击一键快速保存神签图 ✨"}
                </span>
              </motion.div>
              ...
              <div className="mt-8 flex flex-row items-center justify-center gap-3 w-full max-w-sm sm:max-w-md mx-auto px-4">
                {/* Immersive Modal Share Trigger */}
                <button
                  onClick={handleOpenShare}
                  className="flex-1 group relative cursor-pointer pointer-events-auto active:scale-95 transition-all text-amber-100/90"
                >
                  <div className="relative w-full h-11 bg-white/[0.03] border border-white/10 hover:border-amber-500/25 hover:bg-amber-500/5 hover:text-amber-200 text-stone-200 text-[11px] sm:text-[12px] uppercase tracking-[0.15em] font-semibold rounded-full overflow-hidden flex items-center justify-center gap-1.5 shadow-lg group">
                    <Share2 className="w-3.5 h-3.5 text-stone-300 group-hover:text-amber-400 shrink-0" />
                    <span className="font-serif">画卷分享福缘</span>
                  </div>
                </button>

                <button
                  onClick={handleReset}
                  className="flex-1 flex items-center justify-center h-11 space-x-1.5 rounded-full bg-white/[0.03] border border-white/10 hover:border-amber-500/25 hover:bg-amber-500/5 hover:text-amber-200 text-stone-200 text-[11px] sm:text-[12px] font-serif font-medium tracking-wider transition-all duration-200 cursor-pointer pointer-events-auto shadow-lg active:scale-95 group"
                >
                  <Undo2 className="w-3.5 h-3.5 text-stone-300 group-hover:text-amber-400 group-hover:animate-pulse shrink-0" />
                  <span className="font-serif">再度占叩心愿</span>
                </button>
              </div>
                ...
                {isGenerating ? (
                  ...
                  <div className="flex flex-col items-center space-y-3 w-full">
                    ...
                    <div className="relative max-w-full group">
                     ...
                     {/* Prompt banner to save image based on device */}
                     <div className="flex items-center justify-center pt-1.5">
                       {getDevice().isIos ? (
                         <div className="flex items-center space-x-1.5 px-3.5 py-1.5 bg-amber-500/5 border border-amber-500/20 rounded-full text-center select-none">
                         ...
                         <button
                           onClick={() => handleSaveToLocal(generatedImage, '答案之书_命运神签_古典画卷.png')}
                           className="flex items-center space-x-1.5 px-4 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 active:scale-[0.98] border border-amber-500/20 rounded-full text-center cursor-pointer transition-all duration-200 group shadow-lg pointer-events-auto"
                         >
                           <Download className="w-3.5 h-3.5 text-amber-400 animate-pulse shrink-0 group-hover:scale-110" />
                           <span className="text-[10px] sm:text-[11px] font-serif text-amber-200 tracking-wider font-semibold">
                             点击一键快速保存神签图
                           </span>
                         </button>
                       )}
              ...
              <div className="space-y-3 border-t border-white/5 pt-3">
                ...
                <div className="flex justify-center items-center gap-5 sm:gap-7 pt-1 flex-wrap">
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    <button
                      onClick={async () => {
                        const { isWechat, isIos } = getDevice();
                        const shareWords = '天意昭示！我在这里占问得一叶心愿仙签，句句灵验入骨。你来占算卜问你的一生福报与心缘归处：' + window.location.href;
                        
                        await copyTextToClipboard(shareWords);
                        grantShareBonus();

                        if (isWechat) {
                          showToast('💡 签语及分享链接已自动复制！请轻触右上角【⋯】选择「发送给朋友」直接发送给好友同沾福缘 ✨');
                        } else {
                          if (navigator.share) {
                            try {
                              await navigator.share({
                                title: '答案之书 · 命运神签',
                                text: '天意昭示！我在这里占问得一叶...仙签：',
                                url: window.location.href
                              });
                            } catch (e) {
                              showToast('🔮 链接与签语文案已复制成功！请打开微信手动分享与粘贴发送给好友吧 ✨');
                            }
                          } else {
                            showToast('🔮 您的设备不支持直接系统分享，链接与分享口令已复制成功！请打开微信手动粘贴发送给好友同沾福报 ✨');
                          }
                        }
                      }}
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    <button
                      onClick={async () => {
                        const { isWechat } = getDevice();
                        const momentWords = '天意昭示！我在这里占问得一叶心愿仙签，句句灵验入骨。汝亦来占问此生福报与心愿归处吧 🌟 ' + window.location.href;
                        
                        await copyTextToClipboard(momentWords);
                        grantShareBonus();

                        if (isWechat) {
                          showToast('💡 朋友圈分享文案已自动复制！请轻触右上角【⋯】并选择「分享到朋友圈」粘贴配文直接发布，同沾福报 ✨');
                        } else {
                          showToast('💡 朋友圈分享文案已自动复制！请保存下方神签古典画卷到相册，并在朋友圈手动发图贴文，同沾福报 ✨');
                        }
                      }}
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    <button
                      onClick={async () => {
                        ...
                        if (isWechat) {
                          showToast('📕 微信环境内无法直接拉起小红书，请轻触右上角【⋯】并选择「在浏览器打开」即可启动一键跳转发布功能哦 ✨');
                          return;
                        }

                        const xhsWords = '仙签绝准，占问心愿！今日倾心占得一签，万般宿命皆有定数，诚心问卜，终得妙悟。\n#答案之书 #今日运势 #古典占卜 #命运神签 #今日占卜 ' + window.location.href;
                        
                        await copyTextToClipboard(xhsWords);
                        grantShareBonus();

                        showToast('📕 小红书精美配文已复制！古典画卷即将为您启动保存，随后自动拉起小红书发布页 ✨');

                        setTimeout(() => {
                          if (!isIos && generatedImage) {
                            handleSaveToLocal(generatedImage, '答案之书_命运神签_古典画卷.png');
                          }
                        }, 400);

                        setTimeout(() => {
                          const scheme = isIos ? 'xhsdiscover://post_note/' : 'xhsdiscovery://post_note/';
      ...
      {/* --- iOS / Special Mobile Web Browser Long-Press Save Preview Mask --- */}
      <AnimatePresence>
        {showSavePreview && previewSaveImage && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[110] bg-stone-950/95 backdrop-blur-md flex flex-col items-center justify-center p-4 cursor-pointer select-none"
          ...
          >
            <motion.div
              ...
              transition={{ type: 'spring', damping: 25, stiffness: 350 }}
              className="relative max-w-full sm:max-w-md flex flex-col items-center space-y-4"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Close Button on top-right */}
              <button
                onClick={() => setShowSavePreview(false)}
                className="absolute -top-12 right-2 rounded-full p-2 bg-white/10 hover:bg-white/20 text-white/80 hover:text-white transition-all cursor-pointer z-50 border border-white/10 shadow-lg"
              >
                <X className="w-5 h-5" />
              </button>

              {/* Real high-res img */}
              <div className="relative group rounded-2xl overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.8),_0_0_30px_rgba(217,119,6,0.15)] border border-amber-500/20 max-h-[70vh] w-full">
                <img
                  referrerPolicy="no-referrer"
                  src={previewSaveImage}
                  alt="长按保存命运神签"
                  className="max-h-[70vh] object-contain w-full select-auto pointer-events-auto touch-pan-y"
                  style={{ 
                    WebkitTouchCallout: 'default', 
              ...
              {/* Instructions block */}
              <div className="text-center space-y-2 pointer-events-none select-none max-w-xs sm:max-w-md pt-2 px-4">
                <div className="inline-flex items-center space-x-1.5 px-3.5 py-1 bg-amber-500/15 border border-amber-500/20 rounded-full">
                  <Fingerprint className="w-4 h-4 text-amber-400 animate-pulse" />
                  <span className="text-[11px] font-serif text-amber-200 tracking-wider">
                    请在上方图片上「长按」进行保存
                  </span>
                </div>
                <p className="text-[10px] text-zinc-400 font-sans tracking-wide">
                  由于手机系统与部分浏览器限制，请执行长按并选择「保存图片/添加到照片」即可存入系统相册哦 ✨
                </p>
                <p className="text-[10px] text-zinc-500 font-sans">
                  提示：点击任意空白区域或右上角即可关闭此预览页
                </p>
              </div>
            </motion.div>
          </motion.div>
